...
if st.session_state.custom_problems:
    ...
    if correct_answer:
        ...
        if submit1:
            ...
            st.session_state[f"is_first_attempt_correct_{selected}"] = (user_answer.strip() == correct_answer.strip())

        is_first_correct = st.session_state.get(f"is_first_attempt_correct_{selected}", None)
        if is_first_correct is True:
            st.success("⭕ 정답입니다!")
            ...
            if submit2:
                ...
                st.session_state[f"is_fix_correct_{selected}"] = (fix.strip() == correct_answer.strip())

            is_fix_correct = st.session_state.get(f"is_fix_correct_{selected}", None)
            if is_fix_correct is True:
